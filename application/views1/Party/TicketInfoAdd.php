									...
									<div style="padding-top: 5px">
										<select class="form-control" id="party_name" name="party_name" disabled>
											...
											<?php if(isset($PartyName)) { foreach ($PartyName as $key) { ?>
												<option value="<?php echo $key['id']; ?>" <?php if($key['id'] == $this->session->userdata('party_id')){ echo "selected"; } ?>><?php echo $key['name']; ?></option>
											<?php } } ?>
			                    		</select>
		                    		</div>		
				                </div>
									...
									<div>
										<select class="form-control select2" id="machine_serial_no" name="machine_serial_no">
											...
											<?php if(isset($MachineSerial)) { foreach ($MachineSerial as $key) { ?>
												<option value="<?php echo $key['amc_machine_id']; ?>"><?php echo $key['serial_no']." (".$key['item_company_info_name']." -".$key['model_no'].")"; ?></option>
											<?php } } ?>
			                    		</select>
		                    		</div>		
				                </div>
...
<script type="text/javascript">
	...
	$("#party_group").change(function(){
    	var partyGroup = $("#party_group").val();
      	...
      	$.ajax({
	        url: '<?php echo base_url(); ?>Party/TicketInfo/CheckMachineSerial',
	        type: "POST",
	        data: {"partyGroupId":partyGroup},
	        success: function (result) {
