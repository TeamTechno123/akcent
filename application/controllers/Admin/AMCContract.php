...
class AMCContract extends CI_Controller {
	...
	public function AMCContractDelete($id){
		...
		$data = $this->Admin_model->DeleteCheckAMCContract($id);
		if(empty($data)){
			$data = $this->Admin_model->DeleteCheckPartyWiseInfo($id);
			
			if(empty($data)){
				$data = $this->Admin_model->DeleteCheckSaleInvoiceAMCContract($id);
				if(empty($data)){
