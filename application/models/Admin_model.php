...
class Admin_model extends CI_Model {
	...
	public function DeleteCheckPartyInfo($id){
        $this->db->where('party_info_id', $id);
	...
	public function DeleteCheckItemCom($id){
		$this->db->where('item_company_id', $id);
	...
	public function DeleteCheckIntemGroup($id){
	...
	public function DeleteCheckItemInfo($id){
	...
	public function DeleteCheckUnit($id){
	...
	public function DeleteCheckTaxSlab($id){
		$this->db->where('taxslab_id', $id);
		$query = $this->db->get('item_info');
        if ($query->num_rows() > 0) {
            return $query->result_array();
        } else {
	...
	public function DeleteCheckAMCType($id){
	...
	public function DeleteCheckAMCContract($id){
		$this->db->where('amc_contract_information_id', $id);
		$query = $this->db->get('amc_machine_details');
        if ($query->num_rows() > 0) {
            return $query->result_array();
        } else {
	...
	public function DeleteCheckPartyWiseInfo($id){
	...
	public function DeleteCheckSaleInvoiceAMCContract($id){
		$this->db->where('amc_contract_info', $id);
		$query = $this->db->get('sale_invoice');
        if ($query->num_rows() > 0) {
            return $query->result_array();
        } else {
	...
	public function DeleteCheckTicketInfo($id){
    ...
    public function DeleteCheckSaleInvoice($id){
    	$this->db->like('due_bills', $id);
	...
	public function GetReceiptInfo($id){
		$this->db->select('receipt_info.*, party.party_name, party_info.name, amc_contract_info.*');
		if($id !== null){
        	$this->db->where('receipt_info.id', $id);
        }
		$this->db->join('party', 'party.id = receipt_info.party_id','LEFT');
		$this->db->join('party_info', 'party_info.id = receipt_info.party_info_id','LEFT');
		$this->db->join('amc_contract_info', 'amc_contract_info.contract_id = receipt_info.amc_contract_info','LEFT');
